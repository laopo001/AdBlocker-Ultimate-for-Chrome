...
var Adguard = function () {
	...
	var constants = {
		...
		iframe: {//maximum values for all browsers was leaved for compatibility
			...
			detailedMenuHeight: 300,
			selectorMenuHeight: 115,
			topOffset: 25
		}
	};
	...
	var loadDefaultScriptsAndStyles = function (iframe, dfd) {
		...
		contentPage.sendMessage({type: 'loadAssistant'}, function (response) {
			...
			var cssSelection = response.cssSelection;
			...
			if (cssLink) {
				$.each(cssLink, function (i, cssUrl){
	...
	var createAdguardDetailedMenu = function () {
		return $('<div />', {'class': 'ultimate-container'})
			.append($('<div />', {'class': 'header'})
				.append($('<div />', {'class': 'ultimate-close', 'text': 'X'}))
				.append($('<div />', {'id': 'drag-handle'})
					.append($('<div />', {'class': 'header-h', 'i18n': 'assistant_block_element'}))
					.append($('<div />', {'class': 'head txt'})
						.append($('<span />', {'i18n': 'assistant_block_element_explain'}))
						)
					)
				)
			.append($('<div />', {'id': 'adv-settings', 'class': 'ultimate-form-block'})
				.append($('<div />', {'class': 'clearfix', 'id': 'one-domain-p'})
					.append($('<input />', {'type': 'checkbox', 'id': 'oneDomainRadio'}))
					.append($('<label />', {'for': 'oneDomainRadio', 'class': 'box'}))
					.append($('<span />', {'class': 'checkbox-label', 'i18n': 'assistant_apply_rule_to_all_sites'}))
					)
				.append($('<div />', {'class': 'clearfix', 'id': 'block-by-url-p'})
					.append($('<input />', {'type': 'checkbox', 'id': 'blockByUrl'}))
					.append($('<label />', {'for': 'blockByUrl', 'class': 'box'}))
					.append($('<span />', {'class': 'checkbox-label', 'i18n': 'assistant_block_by_reference'}))
					)
				.append($('<div />', {'class': 'clearfix', 'id': 'block-similar-p'})
					.append($('<input />', {'type': 'checkbox', 'id': 'blockSimilar'}))
					.append($('<label />', {'for': 'blockSimilar', 'class': 'box'}))
					.append($('<span />', {'class': 'checkbox-label', 'i18n': 'assistant_block_similar'}))
					)
				.append($('<p />')
					.append($('<input />', {'id': 'filter-rule', 'type': 'type'}))
					)
				)
			.append($('<div />', {'class': 'ultimate-foot clearfix2r'})
				.append($('<a />', {'id': 'ultimate-accept', 'class': 'btn btn-blue'})
					.append($('<span />', {'i18n': 'assistant_block'}))
					)
				.append($('<a />', {'id': 'ultimate-cancel', 'class': 'btn btn-gray f-right'})
					.append($('<span />', {'i18n': 'assistant_another_element'}))
					)
				.append($('<a />', {'id': 'ultimate-preview', 'class': 'btn btn-gray f-right'})
					.append($('<span />', {'i18n': 'assistant_preview'}))
					)
				)

	};
	var createAdguardSelectorMenu = function () {
			return $('<div />', {'class': 'ultimate-container ultimate-adblock'})
				.append($('<div />', {'class': 'header'})
					.append($('<div />', {'id': 'close-button', 'class': 'ultimate-close', 'text': 'X'}))
					.append($('<div />', {'id': 'drag-handle'})
						.append($('<div />', {'class': 'header-h', 'i18n': 'assistant_select_element'}))
						.append($('<div />', {'class': 'head txt'})
	...
	var showDetailedMenu = function () {
		...
		showDialog(content, constants.iframe.baseWidth, constants.iframe.detailedMenuHeight, function (iframe) {
			bindClicks(iframe, {
				...
				'#ultimate-cancel': onSelectElementClicked,
				'#ultimate-preview': onRulePreview,
