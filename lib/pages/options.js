...
PageController.prototype = {
    ...
    _bindEvents: function () {
        ...
        this.userFilters = $("#userFiltersBox");
        this.wlFilters = $("#excludedDomainsBox");
        ...
        this.subscriptionModalEl = $('#addSubscriptionContainer');
        ...
        $('#startSubscriptionSelection').on('click', this._openSubscriptionModal.bind(this));

        var listSettings = $(".settings-page-lists");
        listSettings.on('click', '#addWhitelist', this.onAddWhiteListFilterClicked.bind(this));
        listSettings.on('click', '#addCustomFilter', this.onAddUserFilterClicked.bind(this));
        ...
        $('#addWhitelist').on('click', function(e){
            e.preventDefault();
            this._importWhiteListFilterRules($('#newWhitelistDomain').val());
            $('#newWhitelistDomain').val('');
            //zz
        }.bind(this));
        $('#removeWhitelist').on('click', function(e){
             e.preventDefault();
            var f;
            $($('#excludedDomainsBox').val()).each(function(k, filter){
                f = filter;
                contentPage.sendMessage({type: 'removeWhiteListDomain', text: filter});
            })
            this._removeEditableFilter('#excludedDomainsBox', f);
        }.bind(this));

        $('#addCustomFilter').on('click', function(e){
            e.preventDefault();
            this._importUserFilterRules($('#newFilter').val());
            $('#newFilter').val('');
        }.bind(this));
        $('#removeCustomFilter').on('click', function(e){
            e.preventDefault();
            var f;
            $($('#userFiltersBox').val()).each(function(k, filter){
                f = filter;
                contentPage.sendMessage({type: 'removeUserFilter', text: filter});
            })
            this._removeEditableFilter('#userFiltersBox', f);
        }.bind(this));
...
//this._renderSearchFilters(/*this.userFilterSearchInput, */this.userFilters, this.clearUserFilterButton, this.userSearchResult, this._renderUserFilter, 'getUserFilters', loadNext);
        ...
        sResult.searchMode = false;//text.length > 0;
    ...
    _renderWhiteListFilter: function (whiteListFilter) {
         $('#excludedDomainsBox').append($('<option>', {text: whiteListFilter}));
         /*
    ...
    _renderUserFilter: function (userFilter) {
        $('#userFiltersBox').append($('<option>', {text: userFilter}));
        /*
        var saveCallback = function (item) {
            if (item.isNew) {
                this.omitRenderEventsCount = 1;
    ...
    _removeEditableFilter: function (listEl, rule) {//(listEl, ruleText, clearButton, searchMode) {
        $(listEl+' option').each(function(){
            if(rule.indexOf($(this).html()) > -1){
                $(this).remove();
                return false;
            }
        })
    ...
    _renderAntiBannerFilters: function () {
        contentPage.sendMessage({type: 'getAntiBannerFiltersForOptionsPage'}, function (response) {
            ...
            $('.subscriptionRemoveButton').on('click', function(){
                var filterId = $(this).parent().find('input.subscriptionEnabled').val();
                contentPage.sendMessage({type: 'removeAntiBannerFilter', filterId: filterId});
                $(this).parent().remove();
            });
        }.bind(this));
    },
    ...
    _renderFiltersMetadataModal: function () {
        ...
        contentPage.sendMessage({type: 'getFiltersMetadata'}, function (response) {
            ...
            for (var i = 0; i < groups.length; i++) {
                ...
                $('#subscriptionSelector').html('');
                for (var j = 0; j < filters.length; j++) {
                    var filter = filters[j];
                    allFilters.push(filter);
                    var filterElement = this._getFilterMetadataTemplate(filter.filterId, filter.name);
                    $('#subscriptionSelector').append(filterElement);
                }
            }
/*
            var homepageText = i18n.getMessage('options_modal_homepage');
...
*/
    ...
    _initializeSubscriptionModal: function () {
        ...
        $(window).on('beforeunload', function () {
            if (this.listenerId) {
                contentPage.sendMessage({type: 'removeListener', listenerId: this.listenerId});
                this.listenerId = false;
            }
        }.bind(this));
        ...
        this.subscriptionModalEl.find('#addSubscription').on('click', function (e) {

            e.preventDefault();
            var selectedFilterIds = [];
            //$.each(this.subscriptionModalEl.find('input[name="modalFilterId"]:checked'), function () {
            $.each(this.subscriptionModalEl.find('option[name="modalFilterId"]:checked'), function () {
    ...
    _getAntiBannerFilterTemplate: function (filterId, version, description) {
        return $('<div>', {class: 's-page-table-row cf subscription'})
                    .append($('<button>', {class: 'i18n_filters_subscription_delete_label subscriptionRemoveButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only', role:'button', 'aria-disabled': 'false', title:'Delete', text:'×'}))
                    .append($('<label>', {class: 'subscriptionEnabledContainer'})
                        .append($('<input>', {class: 'subscriptionEnabled sp-table-row-input', type: 'checkbox', name: 'filterId', value: filterId}))
                        .append($('<span>', {class: 'i18n_filters_subscription_enabled_label', text: i18n.getMessage("options_enabled")}))
                        )
                    .append($('<a>', {class: 'subscriptionTitle', text: description}))
                    .append($('<span>', {class: 'subscriptionUpdate', text: version}))
    },
    ...
    _getFilterMetadataTemplate: function (filterId, filterName) {
        return $('<option>', {text: filterName, name: 'modalFilterId', value: filterId});
    },
...
/****************
    _startSubscriptionSelection: function(title, url)
    {
    ...
    _updateSubscriptionSelection: function()
    {
      var list = document.getElementById("subscriptionSelector");
    ...
    _doAddSubscription: function(url, title, homepage)
    {
