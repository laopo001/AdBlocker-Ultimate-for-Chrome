    ...
    handleMessage: function (message, sender, callback) {
        switch (message.type) {
            case 'updateUserRated':
                this.framesMap.pageStatistic.updateUserRated(message.val);
                break;
            case 'showBadgeAgain':
            ...
            case 'getShieldLocalization':
                ...
                callback({'message': i18n.getMessage(message.key, message.value), 'key': message.key});
                break;
            case 'addUserRule':
            ...
            case 'checkSubscriptionUrl':
                ...
                if (filterMetadata) {
                    //ok, filter found
                    confirmText = i18n.getMessage('abu_subscribe_confirm_enable', [filterMetadata.name]);
                } else {
                    //filter not found
                    confirmText = i18n.getMessage('abu_subscribe_confirm_import', [message.title]);
                }
            ...
            case 'enableSubscription':
                this.antiBannerService.processAbpSubscriptionUrl(message.url, function (rulesAddedCount) {
                    callback({
                        title: i18n.getMessage('abu_subscribe_confirm_import_finished_title'),
                        text: i18n.getMessage('abu_subscribe_confirm_import_finished_text', [rulesAddedCount])
                    });
                });
                return true; // Async
            default :
