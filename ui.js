...
var UI = {
	...
	openUserPromotedPanel: function(rank){
		...
		this._getCurrentTab(function (tab) {
			tab.sendMessage({type: 'initShield', rank: rank});
		});
	...
	getUserRank: function(){
		return framesMap.pageStatistic.getUserRank();
	},
	...
	openAssistant: function (selectElement) {
		this._getCurrentTab(function (tab) {
			tab.sendMessage({type: 'initAssistant', options: {selectElement: selectElement, baseUrl: ext.getURL('/')}});
		});
	},

	getAssistantCssOptions: function () {
		return {
			cssLink: [ext.getURL('lib/content-script/assistant/css/select.css'), 
						ext.getURL('lib/content-script/assistant/css/font-awesome.css'),
						ext.getURL('lib/content-script/assistant/css/font-awesome.min.css')
			]
		};
	},
	...
	customizeContextMenu: function (tab) {
		...
		var callbackMappings = {
			...
			'context_abuse_site': function(){
				UI.openTab('https://adblockultimate.com/report.html?url='+encodeURIComponent(tabInfo.url));
			},
